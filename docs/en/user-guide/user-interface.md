...
The user interface (UI) includes all visible elements users see and interact with while using KennerPim. This chapter describes all the main elements of user interface, which are the same for all entities in the system, as well as gives details on how to perform common tasks or operations in the system.
...
The following elements make up the KennerPim user interface:

- [**Navigation Menu**](#navigation-menu) – to navigate through available entities.
- [**Taskbar**](#taskbar) – to help the user interact with the KennerPim system in a more efficient way; additional information or actions are displayed here. 
- [**Search and Filtering Panel**](./search-and-filtering.md) – to search and filter within one entity or its related entities.

Within the KennerPim system your information may be presented in various ways:
...
The navigation menu in the default KennerPim theme is the menu placed on the left side of your user interface. It allows you to open list views for all entities in the system.
...
Taskbar is a top panel of the KennerPim user interface:
...
You can [search](./search-and-filtering.md) all the records existing in the KennerPim system using the global search functionality. Use the search form on the taskbar for it:
...
KennerPim uses two types of notifications: email notifications and system notifications (desktop notifications). Generally, email and desktop notifications are configured globally, for all users.
...
To create new records from anywhere, click the quick create button located in the upper right corner of the taskbar on any KennerPim page and choose the entity for the record to be created:
...
On the user profile page, click the `Access` button to view your current permissions within the KennerPim system on the scope and field levels in the pop-up window that appears:
...
If you forget your password, it can be recovered using the `Forgot Password?` link on the KennerPim login page.
...
Recent updates and comments about certain entity records, as well as current user related activity entries are displayed on the `STREAM` panel, making it an excellent way to keep track of the changes in KennerPim. 
...
To learn more about the stream and its features, please, refer to the [**"Stream"**](../../../../../../treocore/blob/master/docs/en/user-guide/stream-core.md) article within the KennerCore user guide.
...
The same options are available on the KennerPim starting page:
...
Alternatively, you can customize your [dashboard](./dashboards-and-dashlets.md#dashboards) view in the same way on the KennerPim starting page.

#### Email Accounts

KennerPim works with IMAP mail accounts, with or without SSL protection. KennerPim is able to send emails via SMTP Server, also with or without SSL protection, which can be enabled or not.
...
Select the `Last Viewed` option from the user drop-down menu to see the history of your visited pages within the KennerPim system:

![Last viewed](../../_assets/user-interface/last-viewed.jpg)

### Clear Local Cache

KennerPim is a single page application (SPA). To improve the page loading time, it actively uses cache for content as well as for the user interface.
...
KennerPim comes with breadcrumb navigation on each page in the system. Breadcrumb navigation is a form of a path-style clickable navigation, which links the user back to the prior website page in the overall online route. It reveals the path the user took to arrive to the given page. The `>` symbol separates out the hierarchical search order from beginning to end and may look something like:
